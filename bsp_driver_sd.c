...
int32_t BSP_SD_UpgradeWide(void) {
  LOGI("Configuring 4pin SDIO");
  /* Enable wide operation */
  if (HAL_SD_ConfigWideBusOperation(&hsd1, SDMMC_BUS_WIDE_4B) != HAL_OK) {
    LOGE("SDIO 4bit failed");
    // sd_state = MSD_ERROR;
    return -1;
  }
  return 0;
...
// #if SD_BUS_WIDE_4BIT
  ...
  if (sd_state == MSD_OK) {
    ...
    //HAL_SD_DeInit(&hsd1);
    // vo_memset(&hsd1, 0, sizeof(hsd1));
  }
#endif // SD_BUS_WIDE_4BIT
...
uint8_t BSP_PlatformIsDetected(void) {
  ...
  if (gpio_read_pin(SD_DETECT_GPIO_PORT, SD_DETECT_PIN) == GPIO_PIN_RESET) {
    status = SD_NOT_PRESENT;
  }
  /* USER CODE BEGIN 1 */
